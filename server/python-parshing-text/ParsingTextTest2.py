...
response = client.text_detection(image=image, image_context={'language_hints': ['he', 'en']})
...
def fix_hebrew_text(text):
    ...
    words = text.split()
    fixed_words = [word[::-1] if any(0x590 <= ord(c) <= 0x5FF for c in word) else word for word in words]
