...
export const UserProvider = ({ children }) => {
  ...
  const loadUser = async () => {
    try {
      ...
      setUser({
        ...
        folders: folders,
        favorites: favorites, // מוסיף גם את המועדפים
        imgPath: null,
      });
    } catch (error) {
  ...
  const loadFoldersFromDB = async () => {
    ...
    try {
      ...
      return [[], []]; // נחזיר שני ערכים גם בשגיאה
    }

    const favoritesTmp = []; // רשימת מועדפים זמנית
    const folders = await Promise.all(
      foldersDetails.map(async (folder) => {
        ...
        filesList.forEach((file) => {
          ...
          // בדיקה אם הקובץ מסומן כמועדף (נניח שיש לו שדה file.isFavorite === true)
          if (file.isFavorite) {
            favoritesTmp.push({
              fileId: file.id,
  ...
  const markAsFavorite = async (value, fileId, folderName) => {
    ...
    try {
      ...
      setUser((prevUser) => {
        const updatedFolders = { ...prevUser.folders };
        const updatedFavorites = [...(prevUser.favorites || [])]; // לוודא שיש מערך
